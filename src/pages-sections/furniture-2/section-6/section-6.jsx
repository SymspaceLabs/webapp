  ...
  return (
    <Grid sx={{backgroundColor:'#FAFAFA'}}>
      <Container maxWidth="md">
        <Box sx={{ display:'flex', flexDirection:'column', gap:3 , textAlign: 'center', py: 8 }}>
          <Box sx={{ display: 'flex' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent:'space-between', gap:'32px' }} >
              {items.map((item, index) => (
                  <Box sx={{ width: 160, height: 128, backgroundColor: '#EDEDED', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000', borderRadius: '15px' }}>
                    {item}
                  </Box>
              ))}
          </Box>
